...
flowchart TD
  ...
  TaskRouting --> Shell[rote-shell]
  TaskRouting --> DirectBlock[explicit fallback or blocker]
  AdapterCreate --> Config[rote-adapter-config]
  AdapterCreate --> Registry
  Browser --> Reusable
  Shell --> Reusable[new reusable result]
  Workspace --> Reusable
  Workspace --> Done
  Reusable --> Crystallize[rote-flow-crystallization]
  Crystallize --> Plan[semantic crystallization plan]
  Plan -->|pending-capable| PendingWrite[pending write for adapter/browser/mixed]
  PendingWrite --> PendingSave[pending save]
  PendingSave --> SaveDecision{save decision}
  Plan -->|process-only| SaveDecision
  SaveDecision -->|accepted or pre-approved| Author[rote-flow-authoring]
  SaveDecision -->|declined| DeclineDiscard[discard pending state when present]
  DeclineDiscard --> Done
  SaveDecision -->|unclear| AskAgain[ask one yes/no question again]
  AskAgain --> SaveDecision
  Author --> Materialize[scaffold or process-only export]
