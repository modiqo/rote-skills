...
| `rote-browse` | Browser launch choice, page leases, snapshots, slices, waits, human gates, saved auth, replay, and browser play caveats. | `rote-flow-crystallization`, `rote-flow-authoring`, `rote-registry`, `rote`. | Caller with workspace, lease/session, launch mode, snapshot/page refs, human-gate status, saved-auth state, result, replayability signal, and next owner. | Browser result is delivered, human gate blocks, page lease is unrecoverable, action needs approval, or play crystallization owns the next step. |
...
- Target skill: `rote-...`
- User intent: ...
- Current state: play search result, adapter/workspace state, registry/setup state, or browser state
