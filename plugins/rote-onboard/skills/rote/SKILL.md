...
- `@N` references (`rote @2 …`, `rote @3 …`, `$decoded` chains) read **cached responses the
  prior command created on disk**. `@2` cannot exist until the command that produced it
  finished. Fire them in parallel and the later ones reference nothing.
- `rote init` → `cd <workspace>` → `rote POST`/`rote <adapter>_call` is a strict order: the
  session and the workspace must exist before the call.
...
| Validate a `.adapt` archive before install | `rote adapter check <archive-file>`          |
...
**If a flow is found → STOP SEARCHING. The task is already solved — run the flow.**

> **This is a hard gate, not a suggestion.** A matched flow means Step 1 resolved the task.
> Do **NOT** then run `rote explore`, `rote init`, `rote <adapter>_probe`, schema reads, or any
> "let me just confirm" work — every one of those is wasted, and exploring when a flow exists is
> the cardinal rote mistake. Exploration (Step 2) happens **only** in the "no flow found" case.
> `rote explore` itself tells you this in its `@@learn` output: *"If these flows match your
> need, use them instead of exploring!"* — obey it.

To run the matched flow, you need its **path** and its **arguments**. Get **both in one call**
from rote — do not guess the path and do not grep `main.ts` for args.

**1. Get the path AND the parameter contract from `rote flow search --json`.** Re-run the search
you already ran, with `--json`. The JSON gives you the absolute `path` and the full ordered
`parameters` list (name, type, required, default) — everything you need, structurally:

```bash
rote flow search "<your intent>" --json
```

Example output for the matched flow:

```json
{
  "query": "rideshare receipts",
  "total": 1,
  "flows": [
    {
      "name": "retrieve-rideshare-receipts",
      "path": "/Users/you/.rote/flows/modiqo/retrieve-rideshare-receipts/main.ts",
      "parameters": [
        { "name": "start_date", "type": "String", "required": true,  "default": null,  "description": "Start date in YYYY/MM/DD format" },
        { "name": "end_date",   "type": "String", "required": true,  "default": null,  "description": "End date in YYYY/MM/DD format" },
        { "name": "providers",  "type": "String", "required": false, "default": "all", "description": "uber,lyft,waymo or 'all'" }
      ]
    }
  ]
}
...
list --json` carries the same `path` + `parameters` for every installed flow if you need the full
inventory.)
...
**If no flow found** (search returned nothing) → Continue to Step 2.

### Step 2: Discover the Right Adapter

> **Even here, watch for a flow.** `rote explore "<intent>"` returns an `@@flows` table when a
> matching flow exists — and its `@@learn` block says *"If these flows match your need, use them
> instead of exploring!"*. If explore surfaces a flow, **stop and run it** (go back to the
> flow-run steps above: resolve the path via `rote flow list`, read its frontmatter args, run
> from `/tmp` with `rote deno run`). Only continue adapter exploration when **no** flow matched.
...
- **Backup** — the flow + adapters live off-machine; a laptop loss doesn't lose the work.
- **Social feedback & discovery** — community-space artifacts are searchable and pullable by others; you get usage and feedback.
- **Security review** — published artifacts can be reviewed (and, for orgs, gated) before teammates pull them.
...
```bash
# Private org namespace (--private):
rote registry adapter publish <adapter-id> <org-slug> --private              # for EACH adapter
rote registry flow push <flow-path> <org-slug> --check-deps --private

# Community public space (no --private):
rote registry adapter publish <adapter-id> <your-handle>                     # for EACH adapter
rote registry flow push <flow-path> <your-handle> --check-deps
```

**The slug is a BARE namespace, never `namespace/name`.** Push and publish take just the org slug or your handle — the artifact name comes from the adapter manifest `id` / flow frontmatter `name`. The `org/name` form is for *pull* only; passing it to push fails with "Unknown namespace". `adapter publish` has no `--check-deps` flag (adapters have no dependencies to check) — only `flow push` does.

`registry adapter publish` packs the installed adapter by id and pushes it; use it (not `adapter push <path>`) so you push exactly the locally-installed adapter the flow depends on. Personal-namespace pushes require a claimed handle (`rote profile set-handle <handle>`); org pushes require owner/admin/developer role in the org.

**Versioning & re-push (what happens when the artifact already exists).** The registry keys versions on `(name, version)`; it never overwrites and never auto-bumps. Three cases, all handled for you:

- **Unchanged** — if the published latest version has the same content hash (and, for adapters, the same fingerprint), the push reports `already current — skipped` and creates no new version. This is a success, not an error. On a re-push where nothing changed, expect adapters and/or the flow to be skipped — say so plainly ("github adapter already current, skipped").
- **Changed content, version bumped** — the push lands a new version cleanly.
- **Changed content, SAME version** — the registry rejects it with `Version conflict: '<slug>/<name>' version '<v>' already exists`. When you see this, the local artifact changed but its version string didn't. Bump it, then re-push: `rote flow bump <path>` (or `--minor` / `--major`) for the flow, `rote adapter bump <adapter-id>` for an adapter. Ask the user which level (patch/minor/major) if it isn't obvious from what changed.

So a clean re-push of an evolved flow is: bump the changed artifacts → push (unchanged deps skip themselves) → report the resulting versions.

**Report back.** After the pushes complete, report the exact versions pushed (and which were skipped as already-current) for the flow and each adapter. On `QuotaExceeded`, surface the bucket and limit and stop — do not retry. On `PermissionDenied`, the caller lacks a publishing role in that org; surface that rather than guessing another namespace.
...
rote flow fork <parent-flow>      # Fork flow with new params
rote decompile <flow-path>        # Extract command log
