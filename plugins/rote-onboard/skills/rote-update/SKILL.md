...
description: >
  ...
  `rote update` and refreshes generated adapter guidance templates. Use when the user says
...
- Owns: update availability check, update invocation, version confirmation, bundled skill
...
- Stop when: update/check completes, update defers to another installer, skill refresh finishes,
...
- `rote update` updates the `rote` binary in place (typically `~/.local/bin/rote`).
- If `rote update` reports the binary was installed via a package manager (brew) or an editor
...
keep paying as it ages. Skip it if `rote update` errored or deferred to a package manager. e.g. "Updated to {version} — your
